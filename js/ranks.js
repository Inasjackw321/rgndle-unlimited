 ...
 * in percentiles.js — which is generated by playing the day out under the
 * solved optimal policy, so a percentile compares you against perfect re-roll
 * decisions rather than an arbitrary reference player.
 *
 * Because tiers are defined by percentile rather than hard-coded score
 * thresholds, retuning the scoring engine automatically recalibrates them —
 * you only need to regenerate the table.
 */
...
export function describeRarity(percentile) {
  ...
  if (percentile < 0.5) {
    return { top, oneIn, capped, text: `beats ${(percentile * 100).toFixed(1)}% of days` };
  }
